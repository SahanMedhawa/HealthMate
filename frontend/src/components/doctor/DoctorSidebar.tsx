...
const DoctorSidebar: React.FC<DoctorSidebarProps> = ({ sidebarOpen, setSidebarOpen, isMobile }) => {
    ...
    <div className={sidebarClasses}>
      ...
      <div className="flex items-center justify-between h-16 px-4">
        <div className="flex items-center">
          <div className="h-8 w-8 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 flex items-center justify-center text-white font-semibold shadow-sm">D</div>
      ...
      <div className="px-3 py-4 border-t border-gray-200">
        <div className={`flex items-center ${!isOpen && !isMobile ? "justify-center" : "justify-between"}`}>
          <span className={`text-xs text-gray-500 ${!isOpen && !isMobile ? "hidden" : "block"}`}>HealthMate Doctor v1.0</span>
          <div className="flex items-center">
