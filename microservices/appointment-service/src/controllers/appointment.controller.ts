...
export const updateAppointment = async (req: Request, res: Response): Promise<void> => {
    try {
        // Get original for comparison (doesn't affect update)
        ...
        // ENHANCED NOTIFICATION - determines correct action
        let action = "updated";
        if (originalAppt) {
            if (req.body.status === "completed" && originalAppt.status !== "completed") action = "completed";
            else if (req.body.status === "cancelled" && originalAppt.status !== "cancelled") action = "cancelled";
            else if ((req.body.date && req.body.date !== originalAppt.date) || 
                     (req.body.time && req.body.time !== originalAppt.time)) action = "rescheduled";
        }
        
        // Only the notification call is enhanced - original still works
        notifyAppointmentUpdate({ 
            appointmentId: req.params.id, 
            doctorId: appt.doctorId?.toString(), 
            patientId: appt.patientId?.toString(), 
            action,  // Now sends correct email type
            date: appt.date 
        });
